...
const COMPONENT_DEFAULTS: ParticleTextProps = {
  text: "Explore the catalogue →",
  colors: ["#FFFFFF", "#0A5BD0", "#FFFFFF"],
  mode: "onEnter",
  replay: true,
  position: "above",
  particleSize: 10,
...
export function ParticleText(props: ParticleTextProps) {
  ...
  const mcRadius = typeof mouseRadius === "number" ? mouseRadius : 60;
...
export function PixelDriftCTA({
  ...
  return (
    <Link
      href={href}
      className={`group relative inline-flex h-12 w-64 items-center justify-center overflow-hidden rounded-full border border-accent/40 bg-accent/90 px-6 shadow-lg backdrop-blur-md transition-all duration-300 hover:scale-105 hover:border-accent hover:bg-accent hover:shadow-accent/25 ${className}`}
    >
      <ParticleText
        text={text}
